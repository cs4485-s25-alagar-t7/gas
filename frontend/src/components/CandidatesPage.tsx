      ...
      <div className="flex-1 flex flex-col bg-gray-100">
        ...
        <div className="p-8">
              ...
              <tbody>
                {currentCandidates.map((candidate) => (
                  <tr key={candidate._id} className="border-t">
                    ...
                    <td className="p-4">
                      <button
                        className="text-blue-600 hover:underline justify-left"
          ...
          <div className="flex justify-end items-center mt-4 space-x-2">
            <button onClick={() => handlePageChange(1)} disabled={currentPage === 1} className="px-3 py-1 rounded-lg bg-gray-200 hover:bg-gray-300">First</button>
            <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 rounded-lg bg-gray-200 hover:bg-gray-300">Previous</button>
            ...
            <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 rounded-lg bg-gray-200 hover:bg-gray-300">Next</button>
            <button onClick={() => handlePageChange(totalPages)} disabled={currentPage === totalPages} className="px-3 py-1 rounded-lg bg-gray-200 hover:bg-gray-300">Last</button>
          </div>
        </div>
      </div>
      ...
      {selectedCandidate && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded shadow-md w-full max-w-md relative">
            ...
            <p><strong>NetID:</strong> {selectedCandidate.netid}</p>
            <p><strong>GPA:</strong> {selectedCandidate.gpa}</p>
            <p><strong>Major:</strong> {selectedCandidate.major}</p>
            {selectedCandidate.resume && (
              <p>
                <strong>Resume:</strong>{" "}
                <a href={selectedCandidate.resume} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                  View Resume
                </a>
              </p>
            )}
